...
public class ScoreBoardExit : MonoBehaviour
{
    ...
    void Start()
    {
        newScore = points.ToString() + "," + userName;
    ...
    void insertNewScore()
    {
        ...
        if (GetFloatFromString(parts[0]) < points)
        {
            for (int i = 8; i >= 0; i--)
            {
                parts = strings[i].Split(',');
                if ( points >= GetFloatFromString(parts[0]))
                {
                    changepoint = i;
                }
            }
    ...
    float GetFloatFromString(string input)
{
    ...
    public void resetScores()
    {
        scoreList.text = "";
        nameList.text = "";
        string[] blankScores = {"0,DEV", "0,DEV", "0,DEV", "0,DEV", "0,DEV", "0,DEV", "0,DEV", "0,DEV", "0,DEV", "0,DEV"};
        using (StreamWriter writer = File.CreateText(filePath))
        {
            foreach (string s in blankScores)
            {
                writer.WriteLine(s);
            }
        }
